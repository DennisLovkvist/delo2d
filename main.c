...
{    
    ...
    if(delo2d_render_initialize(graphics->render_targets,1) == -1){return -1;} 
    delo2d_matrix_orthographic_projection(graphics->ortho_proj,0.0f,(float)WINDOW_WIDTH,0.0f,(float)WINDOW_HEIGHT,1,-1);    
    
    delo2d_vertex_array_create(&graphics->vertex_array,DELO_QUAD_LIST,1);
...
int game_setup(Graphics *graphics)
{
    ...
    delo2d_define_sprite(&graphics->sprites2[0], 500,300,100,100,0,0,1000,1000,1,graphics->textures[1].width,graphics->textures[1].height,1,1,0);
    delo2d_create_sprite_batch(&graphics->sprite_batch,1);
    delo2d_create_sprite_batch(&graphics->sprite_batch2,1);

    delo2d_sprite_batch_add(&graphics->sprite_batch,&graphics->sprites[0],0);
    delo2d_sprite_batch_add(&graphics->sprite_batch2,&graphics->sprites2[0],0); 
...
void set_and_clear_frame_buffer(Graphics *graphics, unsigned int frame_buffer)
{
...
{   
    set_and_clear_frame_buffer(graphics,0);    
        
    delo2d_vertex_array_set_attrib_pointer(&graphics->vertex_array2);
    delo2d_vertex_array_to_graphics_device(&graphics->vertex_array2,0);
    delo2d_render(&graphics->vertex_array2,graphics->textures,2,graphics->shaders[0]);
